...
export const ACTIVE_LEDGER_STATUSES: CommissionStatus[] = ["temporary", "reconciliation_waiting", "payable", "on_hold"];
const PAID_LEDGER_STATUSES = new Set<CommissionStatus>(["paid"]);
const CANCELLED_STATUSES = new Set(["cancelled", "canceled", "voided", "void", "closed_cancelled"]);
const RETURNED_STATUSES = new Set(["returned", "return", "partially_returned"]);
const REFUNDED_STATUSES = new Set(["refunded", "partially_refunded", "paid_refunded"]);
const REFUSED_STATUSES = new Set(["refused", "failed_delivery", "failed delivery", "delivery_failed", "rejected", "undelivered"]);
const DISPUTED_STATUSES = new Set(["disputed", "chargeback", "fraud_review"]);

export type CommissionOrderSnapshot = Pick<PartnerOrder, "partnerId" | "status" | "eligibleProductRevenue" | "deliveredAt" | "cancelledAt" | "returnedAt" | "disputedAt">;
...
function blockedAmount(order: Pick<PartnerOrder, "eligibleProductRevenue">) { return Math.floor((Math.max(order.eligibleProductRevenue, 0) * DEFAULT_REFERRAL_CTV_COMMISSION_RATE_BPS) / BASIS_POINTS); }

export function isOrderCancelledOrReturned(order: Pick<PartnerOrder, "status" | "cancelledAt" | "returnedAt" | "disputedAt">) {
  const status = lower(order.status);
  return Boolean(order.cancelledAt || order.returnedAt || order.disputedAt || CANCELLED_STATUSES.has(status) || RETURNED_STATUSES.has(status) || REFUNDED_STATUSES.has(status) || REFUSED_STATUSES.has(status) || DISPUTED_STATUSES.has(status));
}

export function getOrderCommissionBlockReason(order: Pick<PartnerOrder, "status" | "cancelledAt" | "returnedAt" | "disputedAt">) {
  const status = lower(order.status);
  if (order.cancelledAt || CANCELLED_STATUSES.has(status)) return "Đơn hàng đã hủy trên Haravan";
  if (order.returnedAt || RETURNED_STATUSES.has(status)) return "Đơn hàng đã hoàn trên Haravan";
  if (REFUNDED_STATUSES.has(status)) return "Đơn hàng đã hoàn tiền trên Haravan";
  if (REFUSED_STATUSES.has(status)) return "Đơn hàng giao thất bại/từ chối nhận";
  if (order.disputedAt || DISPUTED_STATUSES.has(status)) return "Đơn hàng đang tranh chấp trên Haravan";
  return null;
}

export function isOrderCommissionEligible(order: CommissionOrderSnapshot) {
  return Boolean(order.partnerId && order.eligibleProductRevenue > 0 && !isOrderCancelledOrReturned(order));
}

export function displayOrderCommissionStatus(order: Pick<PartnerOrder, "status" | "cancelledAt" | "returnedAt" | "disputedAt">) {
...
export function decideCommissionForOrder(order: CommissionOrderSnapshot, now = new Date()): CommissionDecision {
  ...
  if (blockReason) return { status: "rejected", amount, commissionRateBps: DEFAULT_REFERRAL_CTV_COMMISSION_RATE_BPS, reason: blockReason, availableAt: null };
...
export async function recalculateOrderCommission(orderId: string, now = new Date()): Promise<RecalculateOrderCommissionResult> {
  return db.$transaction(async (tx) => {
    ...
    if (existing && PAID_LEDGER_STATUSES.has(existing.status)) {
      if (isOrderCancelledOrReturned(order)) { const priorManualReview = await tx.adminAuditLog.findFirst({ where: { action: "commission.paid_manual_review_required", entityType: "PartnerCommissionLedger", entityId: existing.id } }); if (!priorManualReview) await tx.adminAuditLog.create({ data: { partnerId: order.partnerId, action: "commission.paid_manual_review_required", entityType: "PartnerCommissionLedger", entityId: existing.id, beforeJson: { status: existing.status, amount: existing.amount }, afterJson: { orderStatus: order.status, blockReason: getOrderCommissionBlockReason(order) }, note: "Đơn đã hủy sau khi hoa hồng đã thanh toán - cần đối soát thủ công." } }); }
      return { orderId, ledger: existing, decision: null, skippedReason: "Paid ledger entries are immutable and were not overwritten. Đơn đã hủy sau khi hoa hồng đã thanh toán - cần đối soát thủ công." };
    }
...
export function summarizeLedgers(ledgers: { status: CommissionStatus; amount: number; order?: Pick<PartnerOrder, "status" | "cancelledAt" | "returnedAt" | "disputedAt"> | null }[]) { const result: Record<CommissionStatus, number> = { temporary: 0, pending_delivery: 0, reconciliation_waiting: 0, payable: 0, paid: 0, rejected: 0, on_hold: 0 }; for (const ledger of ledgers) { const staleBlocked = ledger.order && isOrderCancelledOrReturned(ledger.order) && ACTIVE_LEDGER_STATUSES.includes(ledger.status); result[staleBlocked ? "rejected" : ledger.status] += ledger.amount; } return result; }
