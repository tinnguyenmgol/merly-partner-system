...
function announcementStatusLabel(item: { archivedAt: Date | null; publishAt: Date; expiresAt: Date | null }, now: Date) {
  if (item.archivedAt) return "Đã lưu trữ";
  if (item.publishAt > now) return "Chưa tới giờ xuất bản";
...
export default async function Page() {
  ...
  return <DashboardShell admin><div className="grid gap-6 lg:grid-cols-[1fr_1.2fr]">
    <form method="post" action="/admin/announcements/create" className="card space-y-4">
      ...
      <label className="flex gap-2 text-sm"><input name="pinned" type="checkbox" /> Ghim thông báo</label><label className="flex gap-2 text-sm"><input name="sendEmailToPartners" type="checkbox" /> Gửi email cho CTV/đối tác</label><button className="btn-primary">Tạo thông báo</button>
    </form>
    <div className="card"><h2 className="text-2xl font-bold text-merly-900">Danh sách</h2><div className="mt-4 space-y-3">{announcements.map((item) => <div key={item.id} className="rounded-xl border border-rose-100 p-4"><div className="flex justify-between gap-3"><b>{item.pinned ? "📌 " : ""}{item.title}</b><span className="text-sm text-stone-500">{item.priority} · {item._count.reads} đã đọc</span></div><div className="mt-2"><span className="rounded-full bg-rose-50 px-3 py-1 text-xs font-semibold text-merly-700">{announcementStatusLabel(item, now)}</span></div><p className="mt-2 whitespace-pre-wrap text-sm text-stone-600">{item.body}</p><p className="mt-2 text-xs text-stone-500">Xuất bản {formatVietnamDateTime(item.publishAt)} {item.expiresAt ? `· Hết hạn ${formatVietnamDateTime(item.expiresAt)}` : ""}</p>{!item.archivedAt ? <form method="post" action="/admin/announcements/archive" className="mt-2"><input type="hidden" name="id" value={item.id} /><button className="text-sm font-semibold text-merly-700">Lưu trữ</button></form> : null}</div>)}</div></div>
    <div className="card lg:col-span-2"><h2 className="text-2xl font-bold text-merly-900">Link rút gọn mới tạo</h2><div className="mt-4 overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr><th>CTV</th><th>Short URL</th><th>Đích</th><th>Clicks</th><th /></tr></thead><tbody>{suspiciousLinks.map((link) => <tr className="border-t" key={link.id}><td className="py-3">{link.partner.displayName}</td><td>{shortUrl(link.slug)}</td><td className="max-w-md truncate">{link.destinationUrl}</td><td>{link._count.clicks}</td><td><form method="post" action="/admin/short-links/disable"><input type="hidden" name="id" value={link.id} /><button className="font-semibold text-merly-700">Disable</button></form></td></tr>)}</tbody></table></div></div>
  </div></DashboardShell>;
}
