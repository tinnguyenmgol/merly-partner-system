...
import { archiveCampaignAction } from "@/features/partner-os-actions";

export async function POST(request: Request) {
  const formData = await request.formData();
  try {
    await archiveCampaignAction(formData);
    return NextResponse.redirect(appUrl("/admin/campaigns"));
  } catch (error) {
    ...
    return NextResponse.redirect(appUrl("/admin/campaigns?message=" + message));
  }
}
