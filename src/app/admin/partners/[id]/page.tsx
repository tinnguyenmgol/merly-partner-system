...
export default async function Page({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ statementToken?: string }> }) {
  const { id } = await params;
  const { statementToken } = await searchParams;
  ...
  const partner = await db.partner.findUnique({
    include: { partnerType: true, profile: true, codes: true, ledgerEntries: true, statementTokens: { where: { revokedAt: null }, orderBy: { createdAt: "desc" }, take: 1 }, auditLogs: { orderBy: { createdAt: "desc" }, take: 20 } },
    where: { id },
  });
    ...
    <DashboardShell admin>
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        <div className="card">
          ...
          <p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm font-medium text-amber-800">CTV account/login riêng chưa được triển khai. Hiện chỉ dùng link sao kê bảo mật tạm thời do admin chia sẻ.</p>
