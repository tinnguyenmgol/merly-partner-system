...
import { archiveContentAssetAction } from "@/features/partner-os-actions";

export async function POST(request: Request) {
  const formData = await request.formData();
  try {
    await archiveContentAssetAction(formData);
    return NextResponse.redirect(appUrl("/admin/content-library"));
  } catch (error) {
    ...
    return NextResponse.redirect(appUrl("/admin/content-library?message=" + message));
  }
}
