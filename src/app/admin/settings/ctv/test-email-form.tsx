  ...
  return (
    <form action={action} className="mt-4 grid gap-3 md:grid-cols-[1fr_auto]">
      ...
      <label className="grid gap-2 text-sm font-medium text-stone-700">
        Email nhận thử
        <input className="input" name="to" placeholder="noreply@merlyshoes.com" type="email" required />
      </label>
      <button className="btn-secondary self-end" disabled={pending}>{pending ? "Đang gửi..." : "Gửi email thử"}</button>
    </form>
  );
}
