...
export function RegistrationForm({ partnerRef }: { partnerRef?: string }) {
  ...
  const [wardCode, setWardCode] = useState(state.values.wardCode ?? "");
  ...
  return <form action={formAction} className="mt-6 grid gap-6">
    ...
    {selectedType ? <>
      ...
      <section className="grid gap-4 md:grid-cols-2"><h2 className="md:col-span-2 text-lg font-bold text-merly-900">3. Khu vực hoạt động</h2><label className="grid gap-2 text-sm">Tỉnh/Thành phố *<select className="input" name="provinceCode" required value={provinceCode} onChange={(event) => { setProvinceCode(event.target.value); setWardCode(""); }}><option value="">Chọn tỉnh/thành phố</option>{vietnamAdminUnits.map((item) => <option key={item.code} value={item.code}>{item.name}</option>)}</select></label><input type="hidden" name="provinceName" value={province?.name ?? ""} /><label className="grid gap-2 text-sm">Xã/Phường/Đặc khu{province?.wards.length ? " *" : ""}<select className="input" name="wardCode" required={Boolean(province?.wards.length)} value={wardCode} onChange={(event) => setWardCode(event.target.value)}><option value="">{province?.wards.length ? "Chọn xã/phường/đặc khu" : "Chưa có danh sách phường/xã cho tỉnh này"}</option>{province?.wards.map((ward) => <option key={ward.code} value={ward.code}>{ward.name}</option>)}</select></label>{state.fieldErrors?.provinceCode ? <p className="text-sm font-medium text-red-700 md:col-span-2">{state.fieldErrors.provinceCode}</p> : null}{state.fieldErrors?.wardCode ? <p className="text-sm font-medium text-red-700 md:col-span-2">{state.fieldErrors.wardCode}</p> : null}</section>
